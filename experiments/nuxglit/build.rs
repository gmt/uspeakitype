...
fn main() {
    ...
    let opengl = pkg_config::Config::new()
        .cargo_metadata(true)
        .probe("Qt6OpenGL")
        .expect("Qt6OpenGL pkg-config probe failed");
    let opengl_widgets = pkg_config::Config::new()
        .cargo_metadata(true)
        .probe("Qt6OpenGLWidgets")
        .expect("Qt6OpenGLWidgets pkg-config probe failed");
    ...
    for path in widgets
        ...
        .chain(opengl.include_paths.iter())
        .chain(opengl_widgets.include_paths.iter())
    {
        build.include(path);
    }
